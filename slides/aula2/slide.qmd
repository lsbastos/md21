...
::: incremental
-   Verifique se
$$\sum_x f_x(x) = \sum_{x=0}^n {n \choose k}\theta^x (1-\theta)^{n-x} = 1$$
-   Basta aplicar o teorema binomial
...
A função de probabilidade acumulada de uma variável aleatória contínua é dada por
...
Seja $X$ um ponto aleatoriamente selecionado no intervalo $(a,b)$, e que todos os pontos nesse intervalo são equiprováveis. Ou seja,
$$f_X(x) = \left\{ \begin{array}{ll} k, & x \in (a,b) \\ 0 & c.c.\end{array}\right.$$
...
Seja $X$ uma v.a. com função de probabilidade acumulada $F_X$. A função quantil ou função acumulada inversa é dada por
$$F_X^{-1}(q) = \mathbb{Q}_X(q) = \inf\left\{ x : F_X(x) > q\right\}$$
...
Definição. O **valor esperado**, ou **a média**, ou **o primeiro momento**, de uma variável aleatória $X$ é definido como
$$\mathbb{E}(X) = \left\{ 
\begin{array}{ll} 
...
Seja $k>1$ inteiro. Dizemos que $X$ segue uma uniforme discreta se
$$ f_X(x) = \left\{ 
\begin{array}{ll} 
...
Seja $X$ uma v.a. que representa um evento binário. $X=1$ se o evento ocorre, e $X=0$ se o evento não ocorre. Suponha também que $\theta$ é a probabilidade de ocorrência desse evento. Então dizemos que $X$ segue uma distribuição Bernoulli, $X \sim Bern(\theta)$, se
$$ f_X(x) = \left\{ 
\begin{array}{ll} 
...
Seja $X$ a soma de $n$ experimentos de Bernoulli com probabilidade $\theta$. Dizemos que $X$ segue uma distribuição Binomial, $X \sim Binom(n, \theta)$, se
$$ f_X(x) = \left\{ 
\begin{array}{ll} 
...
Seja $X$ uma variável aleatória que assume valores $0,1,2,\ldots$. Dizemos que $X$ segue uma distribuiçao de Poisson com parâmetro $\theta$, $X \sim Pois(\theta)$, se
$$ f_X(x) = \left\{ 
\begin{array}{ll} 
...
Seja $X$ uma variável aleatória que assume qualquer valor real no intevalo $(a,b)$. Dizemos que $X$ segue uma distribuiçao uniforme no intervalo, $X \sim U(a,b)$, se
$$ f_X(x) = \left\{ 
\begin{array}{ll} 
...
Seja $X$ uma variável aleatória nos reais positivos. Dizemos que $X$ segue uma distribuiçao exponencial com parâmetro $\theta$, $X \sim Exp(\theta)$, se
$$ f_X(x) = \left\{ 
\begin{array}{ll} 
...
- Teorema central do limite, em suas diversas versões, é chave para essa popularidade
- Outros teoremas de inferência reforçam esse popularidade
::: 

## Normal

Seja $X$ uma variável aleatória nos reais. Dizemos que $X$ segue uma distribuiçao Normal, ou distribuição Gaussiana, com parâmetros $(\mu, \sigma^2)$, $X \sim N(\mu, \sigma^2)$, se
