...
. . .
Mas o primeiro momento central, se $\mu<\infty$, é sempre zero.
...
Seja $X \sim Gamma(a,b)$, com $\mathbb{E}[X] =\frac{a}{b}$ e densidade dada por 
$$f_X(x) = \frac{b^a}{\Gamma(a)}x^{a-1}e^{-xb}, \quad, x > 0$$
...
Calcule o valor esperado e a variância das seguintes distribuições:
  ...
  - $X\sim Exp(\lambda)$, 
  $$f_X(x) = \lambda e^{-\lambda}, \, x>0$$
  - $X\sim Beta(a,b)$, 
  $$f_X(x) = \frac{\Gamma(a+b)}{\Gamma(a)\Gamma(b)} x^{a-1}(1-x)^{b-1},\, x\in (0,1)$$
  - $X\sim Normal(\mu, \sigma^2)$, 
  $$f_X(x) = \frac{1}{\sigma \sqrt{2\pi}} e^{-\frac{1}{2\sigma^2} (x-\mu)^2}, \, x\in \mathbb{R}$$
  - $X\sim Binomial(n, p)$, 
  $$f_X(x) = {n \choose x} p^x(1-p)^{n-x}, \, x=0,1,2,\ldots,n$$
  - $X\sim Poisson(\lambda)$, 
  $$f_X(x) = \frac{\lambda^x}{x!} e^{-\lambda}, \, x=0,1,2,\ldots$$
...
$$\mathbb{V}[b] = \mathbb{E}[b^2] - \mathbb{E}[b]^2 = b^2 - b^2 = 0.$$
...
Se $X$ e/ou $Y$ forem discretas, a integral dá lugar a um somatório.
...
De forma mais geral, sejam $X_1, X_2,\ldots, X_n$ v.a.s **independentes** com $\mathbb{V}(X_i) < \infty, \, \forall i$, e $(a_1,b_1), (a_2,b_2),\ldots,(a_n,b_n)$ constantes, então 
$$\mathbb{V}\left(\sum_{i=1}^n (a_i X_i + b_i)\right) = \sum_{i=1}^n a_i^2 \mathbb{V}(X_i)$$
...
$$\mathbb{E}[Y|X=x] = \int y f_{Y|X=x}(y|x) dy = \int_{x}^1 y\frac{1}{1-x} dy$$
$$= \frac{1}{1-x} \left. \frac{y^2}{2}\right|_x^1 = \frac{1-x^2}{2(1-x)} = \frac{1+x}{2}.$$
...
$$\mathbb{E}[\mathbb{E}[X|Y]] = \mathbb{E}[X] \quad \mathbb{E}[\mathbb{E}[Y|X]] = \mathbb{E}[Y]$$
 ...
 Prova. 
 $$ \mathbb{E}[\mathbb{E}[X|Y]] =  \int \mathbb{E}[X|Y=y] f_Y(y) dy \\ = \int \int x f_{X|Y=y}(x|y) dx f_Y(y) dy  $$
...
A variância condicional é definida por 
$$\mathbb{V}(X|Y=y) = \int (X - \mathbb{E}[X|Y])^2 f_{X|Y}(x|y) dx$$
