...
p +
  # xlim(0,4) +
  geom_function(fun = dnorm, args = list(mean = 2, sd =sqrt(1/10)), mapping = aes(colour = "n=10; N(2,1/n)" ) ) +
  geom_function(fun = dnorm, args = list(mean = 2, sd =sqrt(1/30)), mapping = aes(colour = "n=30; N(2,1/n)" ) )  
```
...
- gerar $M=1000$ amostras de tamanho 50 de uma distribuição $Gama(a=4, b=2)$
...
```{r echo=T}
M = 1000
X <- matrix(rgamma(50*M, shape = 4, rate = 2), ncol=M)
...
ggplot() + 
  ...
  geom_function(fun = dnorm, args = list(mean = 2, sd =sqrt(1/50)), mapping = aes(colour = "N(2,1/50)" ) )  +
  theme_bw() +
  labs(
    x = expression(bar(x)),
  ...
  theme(legend.position.inside = T, legend.position = c(0.9, 0.9)) 
```
