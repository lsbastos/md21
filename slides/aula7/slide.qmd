...
$$Z_n = \frac{\sqrt{n}(\bar{X}_n - \mu)}{S_n}  \xrightarrow[n\to \infty]{D} \sim N(0,1).$$

## Para dados normais {.scrollable}
...
A distribuição $t$ de Student (Gosset) com $n-1$ graus de liberdade $$f_{T_n}(t) = \frac{\Gamma(n/2)}{\sqrt{\pi (n-1)}\Gamma((n-1)/2)}\left(1 +\frac{t^2}{n-1}\right)^{-n/2}$$
...
Em muitos casos, queremos saber apenas alguma característica de $F$ como a média por exemplo. S
